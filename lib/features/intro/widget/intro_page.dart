...
import 'package:hiddify/core/model/negern_defaults.dart';
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        onPressed: () async {
          ...
          // Автоматический импорт 6 встроенных подписок Negern VPN.
          // Делаем последовательно с задержкой, чтобы не положить их workers'ы.
          for (final entry in NegernDefaults.subscriptions) {
            try {
              loggy.info("seeding default subscription: ${entry.$1}");
              await ref.read(addProfileNotifierProvider.notifier).addManual(
                    url: entry.$2,
                    userOverride: UserOverride(name: entry.$1),
                  );
              await Future.delayed(const Duration(milliseconds: 400));
            } catch (e, st) {
              loggy.warning("seed default '${entry.$1}' failed", e, st);
            }
          }
